...
def make_ladder_env(paramfile,usecached=False):
    ...
    ladder_angle_rad = ladder_angle*float(pi/180)
    ...
    file_write.write('  <!--<Translation>0 (float(cos(ladder_angle))) 0 </Translation>-->\n')
    file_write.write('    <Translation>0 '+str(2*float(cos(ladder_angle_rad))*stringer_l)+' 0</Translation>\n')
...
def make_ladder(paramfile,usecached=False):
    ...
    if stringer_shape==0:
        ...
        file_write.write('			        <!-- set the translation and rotation of the box. Note that the transformations-->\n')
