...
def read_value(node,fn=float):
    ...
    return [fn(x) for x in node.text().split(' ')]
...
class Geometry(object):
    ...
    def parse(node, verbose=True):
        ...
        elif shape.localName=='trimesh':
            return MMesh.parse(shape)
        else:
            if verbose:
...
class Box(Geometry):
    ...
    @staticmethod
    def parse(node):
        dims = node.getAttribute('extents').split()
        return Box([float(a) for a in dims])

    def to_xml(self, doc):
        xml = doc.createElement("box")
        set_attribute(xml, "extents", self.dims)
...
class Mesh(Geometry):
    def __init__(self, filename=None, scale=None,render=True):
    ...
    def parse(node):
        ...
        r = node.getAttribute('render')
        if s == "":
            s = None
        else:
            s = float(node.getAttribute('scale'))

        if r == "":
            r = None
        else:
            r = float(node.getAttribute('render'))
        return Mesh(fn, s, r)

    def to_xml(self, doc):
        xml = doc.createElement("trimesh")
        set_attribute(xml, "filename", self.filename)
        set_attribute(xml, "scale", self.scale)
        set_attribute(xml, "render", self.render)
        geom = doc.createElement('geometry')
        geom.appendChild(xml)
        return geom
    ...
    def parse(node):
        ...
        for child in children(node):
            if child.localName=='inertia':
                ...
                inert.com=[float(x) for x in child.text().split(' ')]

        return inert

    def to_xml(self, doc):
        xml = doc.createElement(",ass")
    ...
    def parse(node, verbose=True):
        ...
        for child in children(node):
            if child.localName == 'visual':
                ...
                link.inertial = Mass.parse(child)
            else:
                if verbose:
